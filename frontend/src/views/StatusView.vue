  ...
  <section class="card panel status-panel">
    ...
    <section class="card-lite diagnostic-card" v-if="fileCache">
      ...
      <ul class="diag-list">
        ...
        <li><strong>命中率：</strong> {{ formatPercent(fileCache.metrics?.hitRate) }}</li>
        <li><strong>当前来源：</strong> {{ fileCache.overrideActive ? '应用设置覆盖' : '环境变量默认值' }}</li>
      </ul>
      <div class="form-actions">
        <router-link class="btn btn-ghost" to="/cache">打开缓存管理页</router-link>
      </div>
    </section>
